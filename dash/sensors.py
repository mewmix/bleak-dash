...
class RobotSensors(object):
    def __init__(self, client: BleakClient, sensor_state: defaultdict):
        """
        Enable Robot sensors, writing results back into sensor_state dict

        :param client: Bleak connection to robot
    ...
    async def subscribe(self):
        ...
        await self.client.start_notify(CHARACTERISTICS["dot_sensor"], self._dot_sensor_decode)
        await self.client.start_notify(CHARACTERISTICS["dash_sensor"], self._dash_sensor_decode)
    ...
    async def unsubscribe(self):
        ...
        await self.client.stop_notify(CHARACTERISTICS["dot_sensor"])
        await self.client.stop_notify(CHARACTERISTICS["dash_sensor"])
